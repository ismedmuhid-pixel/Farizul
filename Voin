    ...
    <div id="eventModal" class="fixed inset-0 z-[70] hidden flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 transform transition-all scale-95 opacity-0" id="eventContent">
            ...
            <div id="gameSelection">
                <div class="space-y-3">
                    <button onclick="selectGame('number')" class="w-full py-3 bg-primary text-white rounded-lg font-bold hover:bg-teal-600 transition-colors">🧠 Tebak Angka</button>
                    <button onclick="selectGame('rps')" class="w-full py-3 bg-primary text-white rounded-lg font-bold hover:bg-teal-600 transition-colors">✂️ Batu Kertas Gunting</button>
                    <button onclick="selectGame('math')" class="w-full py-3 bg-primary text-white rounded-lg font-bold hover:bg-teal-600 transition-colors">🔢 Kuis Matematika</button>
                    <button onclick="selectGame('riddle')" class="w-full py-3 bg-primary text-white rounded-lg font-bold hover:bg-teal-600 transition-colors">🧩 Tebak Tebakan</button>
                </div>
            </div>
            ...
            container.innerHTML = cats.map(c => `
                <button onclick="filterByCategory('${c}')" class="p-4 rounded-xl bg-white dark:bg-slate-800 border dark:border-slate-700 hover:border-primary dark:hover:border-primary hover:shadow-md transition-all text-center">
                    <div class="text-2xl mb-2">
                        ${c === 'Pakaian' ? '👕' : c === 'Elektronik' ? '⚡' : c === 'Makanan' ? '🍜' : '🎁'}
                    </div>
        ...
        function renderProducts(categoryFilter) {
            ...
            grid.innerHTML = filtered.map((p,idx) => `
                <div class="group product-card cursor-pointer" onclick="openProductModal('${p.id}')">
                    ${p.sold > 20 ? `<div class="product-badge badge-hot">⚡ HOT</div>` : (p.stock < 10 ? `<div class="product-badge badge-limited">📦 LIMITED</div>` : '')}
                    ...
                    <div class="relative aspect-[4/3] card-img-wrap">
                        ...
                        <button onclick="event.stopPropagation(); DB.addToCart('${p.id}')" class="absolute bottom-4 right-4 bg-white p-3 rounded-full shadow-xl hover:bg-primary hover:text-white transition-all transform translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 hover:scale-110">
            ...
            document.getElementById('gameArea').innerHTML = `
                <div class="text-center">
                    ...
                    <div class="flex justify-center gap-4 mb-4">
                        <button onclick="playRPS('rock')" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-teal-600 transition-colors">🪨 Batu</button>
                        <button onclick="playRPS('paper')" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-teal-600 transition-colors">📄 Kertas</button>
                        <button onclick="playRPS('scissors')" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-teal-600 transition-colors">✂️ Gunting</button>
                    </div>
